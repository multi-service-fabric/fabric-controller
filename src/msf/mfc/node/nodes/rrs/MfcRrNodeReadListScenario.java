 ...
 *
 */
public class MfcRrNodeReadListScenario extends MfcAbstractRrNodeScenarioBase<RrNodeRequest> {

  private static final MsfLogger logger = MsfLogger.getInstance(MfcRrNodeReadListScenario.class);
   ...
   *
   */
  public MfcRrNodeReadListScenario(OperationType operationType, SystemInterfaceType systemInterfaceType) {
  ...
  protected RestResponseBase executeImpl() throws MsfException {
    try {
      ...
      try {
        ...
        responseBase = sendRrNodeReadList(request.getClusterId(),
  ...
  private RestResponseBase sendRrNodeReadList(String swClusterId, boolean isDetailList) throws MsfException {
    try {
      ...
      String targetUri = MfcFcRequestUri.RR_NODE_READ_LIST.getUri(swClusterId);
      ...
      RestResponseBase restResponseBase = RestClient.sendRequest(MfcFcRequestUri.RR_NODE_READ_LIST.getHttpMethod(),
          targetUri, null, fcControlAddress, fcControlPort);

      String errorCode = null;
      if (isDetailList) {
        RrNodeReadDetailListResponseBody responseBody = JsonUtil.fromJson(restResponseBase.getResponseBody(),
            RrNodeReadDetailListResponseBody.class, ErrorCode.FC_CONTROL_ERROR);
        errorCode = responseBody.getErrorCode();
      } else {
        RrNodeReadListResponseBody responseBody = JsonUtil.fromJson(restResponseBase.getResponseBody(),
            RrNodeReadListResponseBody.class, ErrorCode.FC_CONTROL_ERROR);
        errorCode = responseBody.getErrorCode();
      }
