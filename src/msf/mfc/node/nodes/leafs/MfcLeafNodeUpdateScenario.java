 ...
 *
 */
public class MfcLeafNodeUpdateScenario extends MfcAbstractLeafNodeScenarioBase<LeafNodeRequest> {

  private static final MsfLogger logger = MsfLogger.getInstance(MfcLeafNodeUpdateScenario.class);
   ...
   *
   */
  public MfcLeafNodeUpdateScenario(OperationType operationType, SystemInterfaceType systemInterfaceType) {
  ...
  protected RestResponseBase executeImpl() throws MsfException {
    try {
      ...
      try {
        ...
        MfcLeafNodeUpdateRunner mfcLeafNodeUpdateRunner = new MfcLeafNodeUpdateRunner(request, requestBody);
        execAsyncRunner(mfcLeafNodeUpdateRunner);

        responseBase = responseLeafNodeUpdateNotifyData();
  ...
  private RestResponseBase responseLeafNodeUpdateNotifyData() {
    try {
      logger.methodStart();
