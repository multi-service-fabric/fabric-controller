 ...
 *
 */
public class MfcPhysicalInterfaceReadListScenario extends MfcAbstractPhysicalInterfaceScenarioBase<PhysicalIfRequest> {

  private static final MsfLogger logger = MsfLogger.getInstance(MfcPhysicalInterfaceReadListScenario.class);
   ...
   *
   */
  public MfcPhysicalInterfaceReadListScenario(OperationType operationType, SystemInterfaceType systemInterfaceType) {
  ...
  protected RestResponseBase executeImpl() throws MsfException {
    try {
      ...
      try {
        ...
        responseBase = sendPhysicalInterfaceReadList(request.getClusterId(),
  ...
  private RestResponseBase sendPhysicalInterfaceReadList(String swClusterId, boolean isDetailList) throws MsfException {
    try {
      ...
      String targetUri = MfcFcRequestUri.PHYSICAL_IF_READ_LIST.getUri(swClusterId, request.getFabricType(),
          request.getNodeId());
      if (isDetailList) {
        targetUri = targetUri + "?format=" + RestFormatOption.DETAIL_LIST.getMessage();
      }
      RestResponseBase restResponseBase = RestClient.sendRequest(MfcFcRequestUri.PHYSICAL_IF_READ_LIST.getHttpMethod(),
          targetUri, null, fcControlAddress, fcControlPort);

      String errorCode = null;
      if (isDetailList) {
        PhysicalIfReadDetailListResponseBody responseBody = JsonUtil.fromJson(restResponseBase.getResponseBody(),
            PhysicalIfReadDetailListResponseBody.class, ErrorCode.FC_CONTROL_ERROR);
        errorCode = responseBody.getErrorCode();
      } else {
        PhysicalIfReadListResponseBody responseBody = JsonUtil.fromJson(restResponseBase.getResponseBody(),
            PhysicalIfReadListResponseBody.class, ErrorCode.FC_CONTROL_ERROR);
        errorCode = responseBody.getErrorCode();
      }
