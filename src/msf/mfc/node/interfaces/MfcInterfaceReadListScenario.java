 ...
 *
 */
public class MfcInterfaceReadListScenario extends MfcAbstractInterfaceScenarioBase<InterfaceRequest> {

  private InterfaceRequest request;

  private static final MsfLogger logger = MsfLogger.getInstance(MfcInterfaceReadListScenario.class);
   ...
   *
   */
  public MfcInterfaceReadListScenario(OperationType operationType, SystemInterfaceType systemInterfaceType) {
  ...
  protected RestResponseBase executeImpl() throws MsfException {
    try {
      ...
      try {
        ...
        responseBase = sendInterfaceReadList(request.getClusterId(),
  ...
  private RestResponseBase sendInterfaceReadList(String swClusterId, boolean isDetailList) throws MsfException {
    try {
      ...
      String targetUri = MfcFcRequestUri.IF_READ_LIST.getUri(swClusterId, request.getFabricType(), request.getNodeId());
      if (isDetailList) {
        targetUri = targetUri + "?format=" + RestFormatOption.DETAIL_LIST.getMessage();
      }
      RestResponseBase restResponseBase = RestClient.sendRequest(MfcFcRequestUri.IF_READ_LIST.getHttpMethod(),
          targetUri, null, fcControlAddress, fcControlPort);

      String errorCode = null;
      if (isDetailList) {
        InterfaceInfoReadDetailListResponseBody responseBody = JsonUtil.fromJson(restResponseBase.getResponseBody(),
            InterfaceInfoReadDetailListResponseBody.class, ErrorCode.FC_CONTROL_ERROR);
        errorCode = responseBody.getErrorCode();
      } else {
        InterfaceInfoReadListResponseBody responseBody = JsonUtil.fromJson(restResponseBase.getResponseBody(),
            InterfaceInfoReadListResponseBody.class, ErrorCode.FC_CONTROL_ERROR);
        errorCode = responseBody.getErrorCode();
      }
