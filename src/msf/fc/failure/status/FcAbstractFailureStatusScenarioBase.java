 ...
 */
public abstract class FcAbstractFailureStatusScenarioBase<T extends RestRequestBase> extends AbstractScenario<T> {
  ...
  protected Map<ClusterType, Map<String, FailureStatus>> getClusterNotifyInfo(SessionWrapper session,
    ...
    try {
      ...
      createClusterTypeMap(session, ifInfoEcMap, clusterId);
      ...
      if (internalLinkMap.size() > 0) {
        ifInfoAllMap = getIfInfoAllMap(ifInfoAllMap, clusterId);
      }
  ...
  private void createClusterTypeMap(SessionWrapper session,
      Map<InterfaceType, Map<String, List<LogicalIfStatusIfEntity>>> ifInfoEcMap, int clusterId) throws MsfException {
  ...
  private void createClusterTypeVlanMap(SessionWrapper session, Map<String, List<LogicalIfStatusIfEntity>> vlanIfMap)
    ...
    for (Map.Entry<String, List<LogicalIfStatusIfEntity>> entry : vlanIfMap.entrySet()) {
      Integer ecNodeId = Integer.valueOf(entry.getKey());
      updateClusterTypeMapFromValn(session, ecNodeId, entry.getValue());
    }
  }

  private void updateClusterTypeMapFromValn(SessionWrapper session, Integer ecNodeId,
      List<LogicalIfStatusIfEntity> logicalIfs) throws MsfException {
  ...
  private Map<InterfaceType, Map<String, List<LogicalIfStatusIfEntity>>> getIfInfoAllMap(
      Map<InterfaceType, Map<String, List<LogicalIfStatusIfEntity>>> ifInfoAllMap, int clusterId) throws MsfException {

    if (ifInfoAllMap != null) {
      return ifInfoAllMap;
    }

    ifInfoAllMap = new HashMap<>();
  ...
  private Map<String, FailureStatus> getInternalFailureMap(SessionWrapper session,
    ...
    Map<LogicalIfStatusIfEntity, LogicalIfStatusIfEntity> internalLinkPairMap = getInternalLinkPairMap(internalLinkMap);

    downLinks = getInternalLinkDownCount(internalLinkPairMap);
  ...
  private Map<LogicalIfStatusIfEntity, LogicalIfStatusIfEntity> getInternalLinkPairMap(
      Map<LogicalIfStatusIfEntity, FcInternalLinkIf> internalLinkMap) throws MsfException {
  ...
  protected List<FailureStatusClusterFailureEntity> getClusterFailureEntityList(
    ...
    if (clusterMap != null) {
      clusters.addAll(getClusterFailureEntityList(clusterMap, clusterId, ClusterType.CLUSTER_LINK_IF));
    }

    Map<String, FailureStatus> edgePointMap = failureInfoMap.get(ClusterType.EDGE_POINT);
    if (edgePointMap != null) {
      clusters.addAll(getClusterFailureEntityList(edgePointMap, clusterId, ClusterType.EDGE_POINT));
    }

    Map<String, FailureStatus> internalMap = failureInfoMap.get(ClusterType.INTERNAL);
    if (internalMap != null) {
      clusters.addAll(getClusterFailureEntityList(internalMap, clusterId, ClusterType.INTERNAL));
    }
    return clusters;
  }
