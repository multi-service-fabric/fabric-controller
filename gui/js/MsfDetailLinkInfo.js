...
"use strict";
    ...
    MSF.MsfDetailLinkInfo.prototype.createRecordsInter = function (baseData, clusterId, clusterLinkId, deviceType, nodeId, ifId) {
        ...
        if (!clusterLinkId && !clusterId) {
            for (i = 0; i < baseData.sw_clusters.length; i++) {
                var clusterId = baseData.sw_clusters[i].cluster_id;
                ...
                for (var j = 0; j < targetIfs.length; j++) {
                    records.push(this.getRecordInter(clusterId, targetIfs[j]));
                }
            }
        } else {
            if (clusterLinkId) {
    ...
    MSF.MsfDetailLinkInfo.prototype.createDetailData = function (iLinkIfId, node, idKey, ifId, internalOptions, nodesInfo) {
        ...
        if (internalOptions == null) {
            ...
            // 対向装置内部リンクIFID
            var opDeviceType = MSF.Const.getFabricTypePlural(internalOptions.opposite_if.fabric_type);
            ...
            var oppositeLinkIfId = this.getOppositeLinkIfId(nodesInfo, opDeviceType, opNodeId, opIfType, opIfId);
