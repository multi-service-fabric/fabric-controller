...
"use strict";
    ...
    MSF.MsfDetail.prototype.updateAllDetailTable = function (baseData) {
        ...
        this.updateTrafficCpTable(baseData);
    ...
    ;
    //
    // 詳細情報テーブル更新処理（8-2_トラヒック情報(CP)）
    //   baseData       : レコード生成に使用する基礎データ(ポーリング取得)
    //   戻り値         : なし
    //
    MSF.MsfDetail.prototype.updateTrafficCpTable = function (baseData) {
        ...
        if (!isMapClusterSelected() && !isMapClusterLinkSelected() && !isNetworkModeNode() && !isControllerMode()) {
            ...
            // レコード生成
            records = this.opTraffic.createRecordsCp(baseData, sliceType, sliceId, clusterId, leafs, deviceType, nodeId, isMap);
        }
        
        // 画面のテーブルへ反映
        this.makeDetailTable(this.opTraffic.TARGET_ID_CP, records, this.opTraffic.COL_NUM_CP);
    }
    ;
