...
"use strict";
    ...
    MSF.MSFMain = function() {
        ...
        // 迂回状態のポーリング実施
        this.isDetourPolling = true;
        // 優先状態のポーリング実施
        this.isPriorityPolling = true;
        // 内部リンクコストのポーリング実施
        this.isPriorityRoutePolling = true;
    ...
    // 通知取得処理
    //
    MSF.MSFMain.prototype.eventPromise = function() {
    
        return this.rest.getNotificationEvent(MSF.MessageInfo.Polling.getNotificationEvent)
        .then(function() {
    ...
    MSF.MSFMain.prototype.Polling = function() {
        ...
        // ※ REST呼び出しを行うので、メイン処理のthenと並行実行
        var clusterId = this.can.viewClusterId;
        var promiseDetour = mainPromise.then(function(response) {
            var promise = response;
            if (this.isDetourPolling){
                promise = Promise.resolve(this.rest.getNodesDetourList(MSF.MessageInfo.Polling.getSystemStatus, this.can.viewClusterId))
                .then(function() {
                    // 描画処理
                    this.can.Draw();
                }.bind(this))
        ...
        // 優先装置グループ情報取得処理
        // ※ REST呼び出しを行うので、メイン処理のthenと並行実行
        var promisePriority = mainPromise.then(function(response) {
            var promise = response;
            if (this.isPriorityPolling){
                promise = Promise.resolve(this.rest.getNodesPriorityList(MSF.MessageInfo.Polling.getSystemStatus, this.can.viewClusterId))
                .then(function() {
                    // 描画処理
                    this.can.Draw();
                }.bind(this))
        ...
        // 内部リンクIF優先度情報一覧取得処理
        // ※ REST呼び出しを行うので、メイン処理のthenと並行実行
        var promisePriorityRoutes = mainPromise.then(function(response) {
            var promise = response;
            if (this.isPriorityRoutePolling){
                promise = Promise.resolve(this.rest.getPriorityRoutesList(MSF.MessageInfo.Polling.getSystemStatus, this.can.viewClusterId))
                .then(function() {
                    // 描画処理
                    this.can.Draw();
                }.bind(this))
        ...
        var lstPromise = [mainPromise, promiseTraffic, promiseController, promiseDetour, promisePriority, promisePriorityRoutes];
        ...
        for (var clusterId in spineAllDic) {
            ...
            for (var spineId in clusterSpines) {
                ...
                ifInfoList.forEach(function(ifInfo) {
                    ...
                    trafficAllDic[clusterId][key].send_rate = spineSendRate.send_rate;
                    trafficAllDic[clusterId][key].send_speed = spineSendRate.speed;
                    trafficAllDic[clusterId][key].receive_rate = leafSendRate.send_rate;
                    trafficAllDic[clusterId][key].receive_speed = leafSendRate.speed;
                }.bind(this));
            }
        }
        ...
        for(var clusterId in clusterInfoDic) {
            ...
            clusterLinkIfs.forEach(function(clusterLinkIf) {
                ...
                for (var i = 0; i < ifTraffic.if_traffics.length; i++) {
                    ...
                    trafficAnimationUpdate(clusterId, opClusterId, traffic, "send");
                    return;
                }
            });
        }
    ...
    ;
    // イベント通知ダイアログ描画
    MSF.MSFMain.prototype.eventStatus = function(noticeType, occurredTime, eventId, restData) {
        var title = i18nx("Main.eventDialog.title", "Attention!!");
        var close = i18nx("Main.eventDialog.close", "OK");
        var buttons = {};
        buttons[close] = function() {
            $(this).dialog('close');
        };
        
        var noticeTypeR   = i18nx("Main.eventDialog.noticeType", "Notice type");
        var occurredTimeR = i18nx("Main.eventDialog.occurredTime", "Occurred time");
        var eventIdR      = i18nx("Main.eventDialog.eventId", "Event id");
        
        var pretty = JSON.stringify(restData, undefined, 2);
        var markup = '<table>'+
                     '  <tr><td>' + noticeTypeR + ':</td><td>' + noticeType + '</td></tr>'+
                     '  <tr><td>' + occurredTimeR + ':</td><td>' + occurredTime + '</td></tr>'+
                     '  <tr><td>' + eventIdR + ':</td><td>' + eventId + '</td></tr>'+
                     '</table>'+
                     '<textarea rows="10" cols="40">' + pretty +
                     '</textarea>';
        $('<div></div>').dialog({
            title: title,
            width: 'auto',
    ...
    MSF.MSFMain.prototype.showMessage = function(Output, operationName, message) {
        switch(Output){
            ...
            case "メッセージボックス":
                ...
                break;
            case "アップグレードパネル":
                this.showInformationPanel(message, i18nx("Main.informationPanel.attentionTitle", "Attention!!"));
                break;
            default:
                break;
        }
    ...
    MSF.MSFMain.prototype.showInformationPanel= function(message, title) {
        ...
        if (!title) title = i18nx("Main.informationPanel.informationTitle", "Information");
    ...
    // セレクトパネル表示
    //
    MSF.MSFMain.prototype.showSelectPanel= function(message, title, yesAction, noAction) {

        if (!title) title = "Select";
        if (!yesAction) yesAction = MSF.main.closeModalPanel;
        if (!noAction ) noAction  = MSF.main.closeModalPanel;

        $("#selectTitle").html(title);
        $("#selectMessage").html(message);

        // ボタンへのアクション追加
        $("#selectYes").click(yesAction);
        $("#selectNo").click(noAction);

        this.showModalPanel("selectPanel", false);
    }
    ;
...
"use strict";
    ...
    if (MSF.Conf.System.Debug.USE_DUMMY_DATA) {
        ...
        setTimeout(function(){
            // 更新用ダミーデータ取得
            var subStatus = MSF.main.db.os_upgrade_substatus;
            // サブ状態をDBに更新or登録
            for(var i = 0; i < subStatus.nodes.length; i++){
                ...
                for(var j = 0; j < MSF.main.db.os_upgrade.length; j++){
                    ...
                    if (MSF.main.db.os_upgrade[j].cluster_id  == subStatus.cluster_id &&
                        ...
                        MSF.main.db.os_upgrade[j].operation_id           = "1";
                        addFlg = false;
                    }
                }
                if (addFlg){
                    var noname = {};
                    noname.cluster_id = subStatus.cluster_id;
                    noname.fabric_type = subStatus.nodes[i].fabric_type;
                    noname.node_id = subStatus.nodes[i].node_id;
                    noname.equipment_type_id = subStatus.nodes[i].equipment_type_id;
                    noname.status = subStatus.nodes[i].status;
                    noname.operator_check_waiting = subStatus.nodes[i].operator_check_waiting;
                    MSF.main.db.os_upgrade.push(noname);
                }
            }
        }, 15000);
        ...
        setTimeout(function(){
            ...
            MSF.main.db.detour_nodes[clusterId] = obj;
            MSF.main.db.detour_nodes;
        }, 5000);
    }
    // --------------------------------------------------------------------
