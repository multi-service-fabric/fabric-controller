...
(function() {
    ...
    MSF.MsfCanvas = function(mainCanvas, animationCanvas, conf) {
        ...
        this.priorityRoutes = {};
        // スイッチ状態アイコン
        this.priorityIcon = new Image();
        this.priorityIcon.src = './img/priority.svg';
        this.detourIcon = new Image();
        this.detourIcon.src = './img/detour.svg';
        this.waitIcon = new Image();
        this.waitIcon.src = './img/wait.svg';
        this.upgradeIcon = new Image();
        this.upgradeIcon.src = './img/upgrade.svg';
        this.errorIcon = new Image();
        this.errorIcon.src = './img/error.svg';
    }
    ;
    //
    // スライス一覧選択状態取得
...
//             MSF.console.debug("アニメーションタイマーが二重に呼ばれました。ID=[" + this.animationTimer + "]" + " ※pastのshowからpresentクリックの場合は問題なし");
    ...
    MSF.MsfCanvas.prototype.setInfo = function(db) {
        ...
        // OSアップグレード情報設定
        this.osUpgrade = db.os_upgrade;

        // 装置迂回情報
        this.detourNodes = db.detour_nodes;

        // 装置優先情報
        this.priorityNodes = db.priority_nodes;
        // リンクコスト
        this.clusterInfoDic = db.clusterInfoDic;
        this.priorityRoutes = db.priority_routes;
...
//             this.LayoutSwitch(0, pcl.SpineList);
    ...
    MSF.MsfCanvas.prototype.Draw = function(mPos) {
        ...
        try {
            ...
            // 凡例
            this.drawColorLegend(ctx, this.Conf.Legend);
            // スライスリスト
            for (var i = 0; i < this.SliceList.length; i++) {
                var si = this.SliceList[i];
                if (!si.isVisible) {
    ...
    MSF.MsfCanvas.prototype.DrawSwitch = function(ctx, sliceInfo, clusterInfo, switchInfo, mPos, conf) {
        ...
        // 装置状態アイコン描画
        if (!si.isLogical) {
            this.drowOSUpgrade(ctx, sw, slide, conf);
            this.drowDetour(ctx, sw, slide, conf);
            this.drowPriority(ctx, sw, slide, conf);
        }
    ...
    MSF.MsfCanvas.prototype.DrawTrafficCpBeard = function(ctx, sliceInfo, switchInfo, size, conf) {
        ...
        for (var i = countOffset; i< count; i++) {
            ...
            ctx.moveTo(x1, y1);
            ctx.strokeStyle = getTrafficColor(trafficValue, "send");
            ...
            ctx.beginPath();
            ctx.moveTo(x2, y2);
            ctx.strokeStyle = getTrafficColor(trafficValue, "receive");
            ctx.setLineDash(this.moveLineInfoList[0].dashList);
            ctx.lineWidth = "2";//conf.STROKE_WIDTH;
            ctx.lineTo((x1+x2)/2, (y1+y2)/2);
            ctx.stroke();
    ...
    MSF.MsfCanvas.prototype.DrawConnectionLine = function(ctx, Spine, Leaf, Alert, conf) {
        ...
        // リンクコスト描画
        this.drowLinkCost(ctx, Spine, Leaf, st, ed);
    }
    ;
    //
    // Spine-Leaf接続線描画処理
    ...
    MSF.MsfCanvas.prototype.DrawTrafficConnectionLine = function(ctx, Spine, Leaf, Alert, conf, traffic) {
        ...
        ctx.beginPath();
        ctx.moveTo(st.x, st.y);
        ctx.strokeStyle = getTrafficColor(traffic, "send");
        //ctx.lineWidth = conf.Active.STROKE_WIDTH;
        ctx.lineWidth = 2;
        ctx.setLineDash(this.moveLineInfoList[0].dashList);
        ...
        ctx.strokeStyle = getTrafficColor(traffic, "receive");
        //ctx.lineWidth = conf.Active.STROKE_WIDTH;
        ctx.lineWidth = 2;
        ctx.setLineDash(this.moveLineInfoList[0].dashList);
        ctx.lineTo(md.x, md.y);
    ...
    // スイッチ優先アイコン描画処理
    //
    MSF.MsfCanvas.prototype.DrawSwitchPriorityIcon = function(ctx, sw, slide, conf) {
        if (MSF.Conf.System.Debug.NOT_NOTIFY_NODE_STATUS) {
            // 障害通知OFFなら処理SKIP
            return;
        }
        // draw icon
        ctx.beginPath();
        ctx.drawImage(this.priorityIcon, 4, -33, 16, 16);
    }
    ;
    //
    // スイッチ迂回アイコン描画処理
    //
    MSF.MsfCanvas.prototype.DrawSwitchDetourIcon = function(ctx, sw, slide, conf) {
        if (MSF.Conf.System.Debug.NOT_NOTIFY_NODE_STATUS) {
            // 障害通知OFFなら処理SKIP
            return;
        }
        // draw icon
        ctx.beginPath();
        ctx.drawImage(this.detourIcon, 3, -34, 18, 18);
    }
    ;
    //
    // スイッチOSアップグレード待ちアイコン描画処理
    //
    MSF.MsfCanvas.prototype.DrawSwitchUpgradeWaitIcon = function(ctx, sw, slide, conf) {
        if (MSF.Conf.System.Debug.NOT_NOTIFY_NODE_STATUS) {
            // 障害通知OFFなら処理SKIP
            return;
        }
        // draw icon
        ctx.beginPath();
        ctx.drawImage(this.waitIcon, 50, -35, 20, 20);
    }
    ;
    //
    // スイッチOSアップグレード中アイコン描画処理
    //
    MSF.MsfCanvas.prototype.DrawSwitchUpgradeIcon = function(ctx, sw, slide, conf) {
        if (MSF.Conf.System.Debug.NOT_NOTIFY_NODE_STATUS) {
            // 障害通知OFFなら処理SKIP
            return;
        }
        // draw icon
        ctx.beginPath();
        ctx.drawImage(this.upgradeIcon, 50, -35, 20, 20);
    }
    ;
    //
    // スイッチOSアップグレード失敗アイコン描画処理
    //
    MSF.MsfCanvas.prototype.DrawSwitchUpgradeErrorIcon = function(ctx, sw, slide, conf) {
        if (MSF.Conf.System.Debug.NOT_NOTIFY_NODE_STATUS) {
            // 障害通知OFFなら処理SKIP
            return;
        }
        // draw icon
        ctx.beginPath();
        ctx.drawImage(this.errorIcon, 50, -35, 20, 20);
    }
    ;
    //
    // スライス障害アイコン描画処理
    ...
    // OSアップグレードアイコンを描画する
    //
    MSF.MsfCanvas.prototype.drowOSUpgrade = function(ctx, sw, slide, conf){
        var swStatus = this.getOSUpgradeStatus(sw);

        switch(swStatus){
            case "unexecuted":
                // 未実行
                this.DrawSwitchUpgradeWaitIcon(ctx, sw, slide, conf);
                break;
            case "detoured":
                // 迂回中
                this.DrawSwitchDetourIcon(ctx, sw, slide, conf);
                break;
            case "os_upgrading":
            case "recovering":
            case "recovered":
                // OSアップグレード中, サービス復旧中, サービス復旧完了
                this.DrawSwitchDetourIcon(ctx, sw, slide, conf);
                this.DrawSwitchUpgradeIcon(ctx, sw, slide, conf);
                break;
            case "failed_not_os_upgraded_not_detoured":
                // 失敗(OS未アップグレード、非迂回状態)
                this.DrawSwitchUpgradeErrorIcon(ctx, sw, slide, conf);
                break;
            case "failed_not_os_upgraded_detoured":
            case "failed_not_recovered":
            case "failed_recovered_detoured":
            case "failed_unknown_detoured":
                // 失敗(OS未アップグレード, サービス未復旧, サービス復旧完了, OSアップグレード不明、迂回状態)
                this.DrawSwitchDetourIcon(ctx, sw, slide, conf);
                this.DrawSwitchUpgradeErrorIcon(ctx, sw, slide, conf);
                break;
            case "completed":
    ...
    // OSアップグレード状況を確認する
    //
    MSF.MsfCanvas.prototype.getOSUpgradeStatus = function(sw){
        var clusterId = this.viewClusterId;
        var nodeId = sw.id;
        var status;
        ...
        var fabricType;
        if (sw.isLeaf) {
            fabricType = MSF.Const.FabricType.Leaf;
        } else {
            fabricType = MSF.Const.FabricType.Spine;
        }

        for(var i = 0; i < this.osUpgrade.length; i++){
            if (clusterId  == this.osUpgrade[i].cluster_id  &&
                fabricType == this.osUpgrade[i].fabric_type &&
                nodeId     == this.osUpgrade[i].node_id){

                // 装置のstatusを返却
                status = this.osUpgrade[i].status;
                return status;
            }
        }
        return status;
    ...
    // 迂回アイコンを描画する
    //
    MSF.MsfCanvas.prototype.drowDetour = function(ctx, sw, slide, conf){
        var nodeIds = [];
        var clusterId = this.viewClusterId;
        var nodeId = sw.id;

        if (!this.detourNodes[clusterId]) return;

        if (sw.isLeaf) {
            nodeIds = this.detourNodes[clusterId].leaf_node_ids;
        } else {
            nodeIds = this.detourNodes[clusterId].spine_node_ids;
        }

        //if (nodeIds.indexOf(Number(nodeId)) >= 0){
    ...
    // 優先アイコンを描画する
    //
    MSF.MsfCanvas.prototype.drowPriority = function(ctx, sw, slide, conf){
        var nodeIds = [];
        var clusterId = this.viewClusterId;
        var nodeId = sw.id;

        if (!this.priorityNodes[clusterId]) return;

        if (sw.isLeaf) {
            nodeIds = this.priorityNodes[clusterId].leaf_node_ids;
        } else {
            nodeIds = this.priorityNodes[clusterId].spine_node_ids;
        }

        //if (nodeIds.indexOf(Number(nodeId)) >= 0){
        if (nodeIds.indexOf(nodeId) >= 0){
            this.DrawSwitchPriorityIcon(ctx, sw, slide, conf);
        }
    }
    ;
    //
    // リンクコストを描画する
    //
    MSF.MsfCanvas.prototype.drowLinkCost = function(ctx, Spine, Leaf, lineSt, lineEd){
        var conf = this.Conf.LinkCost;

        // リンクコスト値取得
        var spineCost = this.getLinkCost(Spine, Leaf, conf);
        var leafCost  = this.getLinkCost(Leaf, Spine, conf);
        // 描画不要の場合、ここで終了
        if ((spineCost < 0) && (leafCost < 0)) {
            return;
        }
        ...
        var spinePt = Point();
        var leafPt  = Point();
        var costWidth  = conf.WIDTH;
        var costHeight = conf.HEIGHT;
        var lineWidth  = lineSt.x - lineEd.x;
        var lineHeight = lineSt.y - lineEd.y;

        if (!(spineCost < 0)) {
            // spine側リンクコストのX座標
            spinePt.x = lineSt.x;
            spinePt.x -= lineWidth / 4;
            spinePt.x -= costWidth / 2;
            // spine側リンクコストのY座標
            spinePt.y = lineSt.y;
            spinePt.y -= lineHeight / 4;
            spinePt.y -= costHeight / 2;
            // リンクコストの枠を描画
            this.drowLinkCostCube(ctx, spinePt, conf);
            // リンクコストの値を描画
            this.drowLinkCostNum(ctx, spinePt, spineCost, conf);
        }

        if (!(leafCost < 0)) {
            // leaf側リンクコストのX座標
            leafPt.x = lineEd.x;
            leafPt.x += lineWidth / 4;
            leafPt.x -= costWidth / 2;
            // leaf側リンクコストのY座標
            leafPt.y = lineEd.y;
            leafPt.y += lineHeight / 4;
            leafPt.y -= costHeight / 2;
            // リンクコストの枠を描画
            this.drowLinkCostCube(ctx, leafPt, conf);
            // リンクコストの値を描画
            this.drowLinkCostNum(ctx, leafPt, leafCost, conf);
        }
    }
    ;
    //
    // リンクコストの枠を描画する
    //
    MSF.MsfCanvas.prototype.drowLinkCostCube = function(ctx, point, conf){
        ctx.beginPath();
        ctx.strokeStyle = conf.STROKE_COLOR;
        ctx.fillStyle = conf.FILL_COLOR;
        ctx.lineWidth = conf.STROKE_WIDTH;

        // アイコンと被らないように3pt上にずらす
        ctx.rect(point.x, point.y - 3, conf.WIDTH, conf.HEIGHT);
        ctx.fill();
        ctx.stroke();
    }
    ;
    ...
    //リンクコストの値を描画する
    //
    MSF.MsfCanvas.prototype.drowLinkCostNum = function(ctx, point, cost, conf){
        // 文字描画位置計算
        var tx = point.x + (conf.WIDTH / 2);
        var ty = point.y + (conf.HEIGHT / 2);

        // リンクコストを描画
        var fontConf = conf.Font;
        ctx.font = fontConf.NAME;
        ctx.textAlign = fontConf.TEXT_ALIGN;
        ctx.lineWidth = fontConf.STROKE_WIDTH;
        ctx.strokeStyle = fontConf.STROKE_COLOR;
        ctx.fillStyle = fontConf.FILL_COLOR;
        ctx.fillText(cost, tx, ty, conf.WIDTH);
    }
    ;
    //
    // 内部リンクコストを取得する
    //
    MSF.MsfCanvas.prototype.getLinkCost = function(myNode, oppoNode, conf){
        var clusterId = this.viewClusterId;
        var myNodeId = myNode.id;
        var myFabricType;
        if (myNode.isLeaf) {
            myFabricType = MSF.Const.FabricType.Leafs;
        } else {
            myFabricType = MSF.Const.FabricType.Spines;
        }

        // DBからリンクコストを取得する
        var nodesInfo = {};
        if (this.priorityRoutes[clusterId] != undefined) {
            nodesInfo = this.priorityRoutes[clusterId][myFabricType] || {};
        }

        var internalLinkIfs = {};
        for (var i = 0; i < nodesInfo.length; i++) {
            if (nodesInfo[i].node_id == myNodeId) {
                internalLinkIfs = nodesInfo[i].internal_link_ifs;
                break;
            }
        }

        var internal_link_if_id = this.getInternalLinkIfId(myNode, oppoNode);
        var cost = -1;
        for (var i = 0; i < internalLinkIfs.length; i++) {
            if (internalLinkIfs[i].internal_link_if_id == internal_link_if_id) {
                cost = internalLinkIfs[i].igp_cost;
                break;
            }
        }

        // 取得したリンクコストがデフォルトの場合
        if (cost == conf.DEFAULT_LINK_COST) {
            cost = -1;
        }
        // 取得したリンクコストが最大表示を超える場合
        if (cost > conf.MAX_LINK_COST) {
            cost = conf.MAX_LINK_COST;
        }
    ...
    // 内部リンクIF IDを取得する
    //
    MSF.MsfCanvas.prototype.getInternalLinkIfId = function(myNode, oppoNode){
        var clusterId = this.viewClusterId;
        var myNodeId = myNode.id;
        var myFabricType;
        if (myNode.isLeaf) {
        ...
        // 装置情報一覧から、自身の装置情報を取得する
        var nodesInfo = this.clusterInfoDic[clusterId].NodesInfo[myFabricType];
        var internalLinkIfs = {};
        var physicalIfs = {};
        var breakoutIfs = {};
        var lagIfs = {};
        for (var i = 0; i < nodesInfo.length; i++){
            if (nodesInfo[i].node_id == myNodeId) {
                internalLinkIfs = nodesInfo[i].internal_link_ifs;
                physicalIfs = nodesInfo[i].physical_ifs;
                breakoutIfs = nodesInfo[i].breakout_ifs;
                lagIfs = nodesInfo[i].lag_ifs;
                break;
            }
        }

        // 内部リンク一覧から、oppoNodeとつながっている内部リンクを取得する
        var internalLinkIfId = -1;
        var internalLinkInfo;
        var oppsiteFlg = false;
        for (var i = 0; i < internalLinkIfs.length; i++){
            internalLinkInfo = internalLinkIfs[i];
            if(internalLinkInfo.physical_if_id != undefined){
                // 物理
                oppsiteFlg = this.checkLink(internalLinkInfo, oppoNode, physicalIfs, "physical_if_id");
            } else if (internalLinkInfo.breakout_if_id != undefined){
                // Breakout
                oppsiteFlg = this.checkLink(internalLinkInfo, oppoNode, breakoutIfs, "breakout_if_id");
            } else if (internalLinkInfo.lag_if_id != undefined){
                // Lag
                oppsiteFlg = this.checkLink(internalLinkInfo, oppoNode, lagIfs, "lag_if_id");
            }
            if (oppsiteFlg) {
                internalLinkIfId = internalLinkInfo.internal_link_if_id;
                break;
            }
        }

        // oppoNodeとつながっている内部リンクのIDを返却
        return internalLinkIfId;
    }
    ;
    //
    // 内部リンク接続チェック
    //  true:oppoNodeとの接続あり false:接続なし
    MSF.MsfCanvas.prototype.checkLink = function(internalLinkInfo, oppoNode, ifs, linkType){
        var opposite;
        for (var i = 0; i < ifs.length; i++) {
            if (internalLinkInfo[linkType] == ifs[i][linkType]) {
                opposite = ifs[i].internal_options.opposite_if;
                break;
            }
        }
        if (opposite != undefined && oppoNode.id == opposite.node_id){
            return true;
        }
        return false;
    }
    ...
    // 凡例描画
    //
    MSF.MsfCanvas.prototype.drawColorLegend = function(ctx, conf){
        //
        // Traffic凡例
        ...
        var fh  = trafficViewLegendFontHeight - 4;
        var canvasAdjust = -2;
        ...
        for (var i = 0; i < trafficColor.length; i++) {
            ...
            ctx.save();
            ctx.beginPath();
            ctx.translate(x, y+vsp*i);
            ctx.rect(0, 0, w, h);
            ctx.fillStyle = trafficColor[index];
            ctx.fill();
            ctx.restore();
            // 凡例ラベル
            ctx.font = conf.Font.NAME;
            ctx.textAlign = conf.Font.TEXT_ALIGN;
            ctx.fillText(trafficBoundary[index], x+w+hsp, y+vsp*i+fh/2+canvasAdjust);
        }
        // 0 表示ラベル
        ctx.fillText("0", x+w+hsp, y+vsp*trafficColor.length+fh/2+canvasAdjust);
        // 表示単位ラベル
        ctx.fillText("[%]", x+w+hsp*2, y+vsp*(trafficColor.length+1)+fh/2+canvasAdjust);

        //
        // アイコン凡例
        var legendSpace = conf.LEGEND_SPACE;
        var iconX   = x;
        var iconY   = y+vsp*(trafficColor.length + 2)+legendSpace;
        var iconW   = conf.ICON_SIZE;
        var iconH   = conf.ICON_SIZE;
        var iconVsp = conf.ICON_SIZE + conf.ICON_SPACE;
        var iconHsp = (w + hsp) - iconW;
        var iconFh  = iconVsp + iconH/2;

        // 凡例アイコン + 凡例ラベル
        // 優先
        ctx.drawImage(this.priorityIcon, iconX, iconY + iconVsp*0, iconW, iconH);
        ctx.fillText(i18nx("Main.legendIcon.priority", "priority"), iconX+iconW+iconHsp, iconY+iconVsp*0+iconFh/2+canvasAdjust);
        // 迂回
        ctx.drawImage(this.detourIcon,   iconX, iconY + iconVsp*1, iconW, iconH);
        ctx.fillText(i18nx("Main.legendIcon.detour", "detour"), iconX+iconW+iconHsp, iconY+iconVsp*1+iconFh/2+canvasAdjust);
        // OSアップグレード待ち
        ctx.drawImage(this.waitIcon,     iconX, iconY + iconVsp*2, iconW, iconH);
        ctx.fillText(i18nx("Main.legendIcon.upgradeWait", "wait OS upgrade"), iconX+iconW+iconHsp, iconY+iconVsp*2+iconFh/2+canvasAdjust);
        // OSアップグレード中
        ctx.drawImage(this.upgradeIcon,  iconX, iconY + iconVsp*3, iconW, iconH);
        ctx.fillText(i18nx("Main.legendIcon.upgradeNow", "now OS upgrade"), iconX+iconW+iconHsp, iconY+iconVsp*3+iconFh/2+canvasAdjust);
        // OSアップグレード失敗
        ctx.drawImage(this.errorIcon,    iconX, iconY + iconVsp*4, iconW, iconH);
        ctx.fillText(i18nx("Main.legendIcon.upgradeFailed", "failed OS upgrade"), iconX+iconW+iconHsp, iconY+iconVsp*4+iconFh/2+canvasAdjust);
    }
    ;
})();
