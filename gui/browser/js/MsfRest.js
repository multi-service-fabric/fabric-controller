...
"use strict";
    ...
    MSF.MsfRest = function(msfCanvas, conf, db) {
        ...
        this.host = conf.HOST;
        this.eventServer = conf.EVENTSERVER;
        this.OperationDic = {};
        this.db = db;
        this.getTimeout = conf.GET_TIMEOUT;
        this.showedLastEventId = 0;
        this.operationCheck = {};
        //
        // 以下デバッグ用設定
        //
    ...
    //
    // Rest通信(イベント受信サーバ)のための構造体を返します
    // messageInfo： 設計書の画面表示メッセージ一覧と対応するMSF.MessageInfoのオブジェクト
    // data：GETのオプションパラメータ または POST等のJsonリクエストBODY
    //
    MSF.MsfRest.prototype.getEventRestStruct = function(type, url, urlParam, messageInfo, data) {
        var restStruct = {
            ajax:{
                // URL作成
                url:this.eventServer + url.format(urlParam),
                // URLパラメータの参照用保持
                urlParam: urlParam,
                // GET、POST等
                type: type,
    ...
    MSF.MsfRest.prototype.getIfTrafficInfo = function (messageInfo, clusterId, fabricType, nodeId) {
        ...
        return this.Rest(rs)
        .then(function(response) {
            var traffics = response.data;
            var clusterInfo = this.db.clusterInfoDic[clusterId];
            var ifInfo = clusterInfo.InterfacesInfo[fabricType][nodeId];
            for (var i = 0; i < traffics.if_traffics.length; i++) {
                var traffic = traffics.if_traffics[i];
                var value = traffic.traffic_value;
                value.speed = getSpeed(ifInfo, value.if_type, value.if_id);
            }
            // DBに設定
    ...
    MSF.MsfRest.prototype.getCPTrafficInfoAll = function(messageInfo) {
        ...
        .then(function(response) {
            ...
            for (i = 0; i < lst.length; i++) {
                id = lst[i].slice_id;
                promiseList.push(this.getCpTrafficInfo(messageInfo, MSF.Const.SliceType.L2, id));
            }
            // L3CP一覧取得
            lst = response[1].data.l3_slices;
            for (i = 0; i < lst.length; i++) {
    ...
    MSF.MsfRest.prototype.getCpTrafficInfo = function (messageInfo, sliceType, sliceId) {
        ...
        .then(function(response) {
            ...
            if (sliceType == MSF.Const.SliceType.L2) {
                this.db.l2cp_traffics[sliceId] = response.data;
                cpTraffics = response.data.l2cp_traffics;
            }
            if (sliceType == MSF.Const.SliceType.L3) {
                this.db.l3cp_traffics[sliceId] = response.data;
                cpTraffics = response.data.l3cp_traffics;
            }
            ...
            cpTraffics.forEach(function(cpTraffic) {
                ...
                cp.speed = this.getCpSpeed(cp);
            }.bind(this));
        }.bind(this));
    };

    // CPスピード取得処理(内部データから取得)
    MSF.MsfRest.prototype.getCpSpeed = function (cpInfo) {

        // CP情報からエッジポイント情報取得
        var clusterInfo = this.db.clusterInfoDic[cpInfo.cluster_id];
        var ep = clusterInfo.EdgepointDic[cpInfo.edge_point_id];

        // エッジポイント情報から速度を取得
        var ifInfo = clusterInfo.InterfacesInfo[MSF.Const.FabricType.Leafs][ep.base_if.leaf_node_id];
        var if_type, if_id;
        if (ep.base_if.physical_if_id) {
            if_type = MSF.Const.RestIfType.PhysicalIf;
            if_id = ep.base_if.physical_if_id;
        }
        if (ep.base_if.breakout_if_id) {
            if_type = MSF.Const.RestIfType.BreakoutIf;
            if_id = ep.base_if.breakout_if_id;
        }
        if (ep.base_if.lag_if_id) {
            if_type = MSF.Const.RestIfType.LagIf;
            if_id = ep.base_if.lag_if_id;
        }
        return getSpeed(ifInfo, if_type, if_id);
    };
    ...
    // 装置迂回情報取得処理 (2018)
    //
    MSF.MsfRest.prototype.getNodesDetourList = function(messageInfo, cluster_id) {
        // polling実施チェック
        if (cluster_id == undefined) return;

        var param = {
            cluster_id:cluster_id,
        };
        var url  = "/v1/detour/clusters/{cluster_id}/nodes";
        var query = {};
        ...
        var promise = this.Rest(rs)
        .then(function(response) {
            MSF.console.info("getNodesDetourList Done. response:" + response.data);
            // DBに設定
            var obj = {};
            obj.leaf_node_ids = response.data.leaf_node_ids || [];
            obj.spine_node_ids = response.data.spine_node_ids || [];

            this.db.detour_nodes[cluster_id] = obj;
    ...
    // 優先装置グループ情報取得処理 (2018)
    //
    MSF.MsfRest.prototype.getNodesPriorityList = function(messageInfo, cluster_id) {
        // polling実施チェック
        if (cluster_id == undefined) return;
        ...
        var url  = "/v1/priority_routes/clusters/{cluster_id}/priority_node_group";
        var query = {};
        ...
        var promise = this.Rest(rs)
        .then(function(response) {
            MSF.console.info("getNodesPriorityList Done. response:" + response.data);
            // DBに設定
            var obj = {};
            obj.leaf_node_ids = response.data.nodes.leaf_node_ids || [];
            obj.spine_node_ids = response.data.nodes.spine_node_ids || [];

            this.db.priority_nodes[cluster_id] = obj;
    ...
    ;
    //
    // 内部リンクIF優先度情報一覧取得処理 (2018)
    //
    MSF.MsfRest.prototype.getPriorityRoutesList = function(messageInfo, cluster_id) {
        // polling実施チェック
        if (cluster_id == undefined) return;
        ...
        var url  = "/v1/priority_routes/clusters/{cluster_id}/nodes";
        var query = {};
        ...
        var promise = this.Rest(rs)
        .then(function(response) {
            MSF.console.info("getPriorityRoutesList Done. response:" + response.data);
            // DBに設定
            var priorityRoutesObj = response.data.nodes || {};

            this.db.priority_routes[cluster_id] = priorityRoutesObj;
    ...
    // 通知取得処理
    //
    MSF.MsfRest.prototype.getNotificationEvent = function(messageInfo) {
        var param = {
            event_id:this.showedLastEventId
        };
        var url = "/v1/event/{event_id}";
        var query = {};

        // Rest通信のための構造体取得
        var rs = this.getEventRestStruct("GET", url, param, messageInfo, query);

        return this.Rest(rs)
        .catch(function(reason) {
            if (MSF.Conf.System.Debug.NOT_CONNECT_TO_EVENT_SERVER) {
        ...
        .then(function(response){
            ...
            MSF.console.info("event server response: \n" + JSON.stringify(isJSON(response.data.events) ? JSON.parse(response.data.events) : response.data.events, null, 4));

            var event = {};
            for (var i = 0; i < response.data.events.length; i++) {
                event = response.data.events[i] || {};
                // イベント内容が取得できない場合は表示させない
                if (event == undefined) continue;
                // 受信しない が設定されている通知は表示させない
                if (MSF.MsfRest.prototype.checkNotifyFlg(event.event_type)) continue;

                if (event.event_type == "upgrade_operations") {
                    // OSアップグレードはモーダルダイアログを表示させる
                    // 障害情報ダイアログ描画
                    MSF.main.rest.operationCheck = event.body || {};
                    if(event.body){
                        var title   = i18nx("Main.osupgrade.Confirm", "Confirm!!");
                        var message = i18nx("Main.osupgrade.Check", "Update Complete!!</br>")
                                    + "&nbsp;&nbsp;[Cluster ID]"
                                    + "<ul><li>" + event.body.cluster_id + "</li></ul>"
                                    + "&nbsp;&nbsp;[Node ID]"
                                    + "<ul><li>" + event.body.fabric_type + "#" + event.body.node_id + "</li></ul>"
                                    + "</br>"
                                    + i18nx("Main.osupgrade.CheckMessage", "Proceed to next switch?");
                        MSF.main.showSelectPanel(message, title, MSF.MSFjsonForm.osUpgradeOperationCheck, MSF.MSFjsonForm.osUpgradeOperationCheckCancel);
                    }
                } else {
                    // それ以外はjQueryのダイアログを表示させる
                    MSF.main.eventStatus(
                        event.event_type,
                        event.occurred_time,
                        event.event_id,
                        event.body
            ...
            MSF.main.rest.showedLastEventId = response.data.last_event_id;
    ...
    //
    // 通知制限チェック
    //true:通知しない false/undefined:通知する
    MSF.MsfRest.prototype.checkNotifyFlg = function(event_type) {
        var notifyFlg = false;
        switch(event_type){
            // オペレーション結果通知
            case "operations":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_OPERATIONS_DIALOG;
                break;
            // コントローラ運用状態通知
            case "status":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_STATUS_DIALOG;
                break;
            // トラヒック情報通知
            case "traffic":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_TRAFFIC_DIALOG;
                break;
            // 障害情報通知
            case "failure_status":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_FAILURE_STATUS_DIALOG;
                break;
            // サイレント故障通知
            case "silent_failure":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_SILENT_FAILURE_DIALOG;
                break;
            // 装置OSアップグレード通知
            case "upgrade_operations":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_UPGRADE_OPERATIONS_DIALOG;
                break;
            // コントローラ異常通知
            case "controller_status_notification/failure":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_CONTROLLER_STATUS_NOTIFICATION_FAILURE_DIALOG;
                break;
            // コントローラログ通知
            case "controller_status_notification/log":
                notifyFlg = MSF.Conf.System.EventServer.NOT_NOTIFY_CONTROLLER_STATUS_NOTIFICATION_LOG_DIALOG;
                break;
            default:
                break;
        }
